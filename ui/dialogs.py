...
class StatsDialog(BaseDialog):
    def __init__(self, db, parent=None):
        ...
        if not stats:
            layout.addWidget(QLabel("暂无标签数据"))
        else:
            for tag, count in stats:
                layout.addWidget(self._progress_bar(tag, count, counts['all']))

        layout.addStretch()

    def _box(self, title, value, color):
        f = QFrame()
        f.setStyleSheet(f"background-color: {COLORS['bg_mid']}; border-radius: 8px; padding: 15px;")
        l = QVBoxLayout(f)
        l.setSpacing(8)

        lbl_title = QLabel(title)
        lbl_title.setStyleSheet(f"color: {COLORS['text_sub']}; font-size: 13px; font-weight: bold; background: transparent;")

        lbl_val = QLabel(str(value))
        lbl_val.setStyleSheet(f"color: {color}; font-size: 26px; font-weight: bold; background: transparent;")

        l.addWidget(lbl_title)
        l.addWidget(lbl_val)
        return f

    def _progress_bar(self, name, val, total):
        p = QProgressBar()
        p.setFormat(f"#{name} ({val})")
        p.setStyleSheet(f"""
            QProgressBar {{
                border: none;
                background-color: {COLORS['bg_mid']};
